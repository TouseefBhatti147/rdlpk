									   ...
									   $connection = Yii::app()->db; 
                                         $sql_test  ="SELECT SUM(total) as cash FROM (
                                            SELECT paidamount AS `total` FROM installpayment 
                                             ...
                                             and plots.project_id='".$key['id']."' and installpayment.paid_date_temp LIKE'".date('Y-m-d')."'
                                            UNION ALL
                                            SELECT paidamount AS `total` FROM  plotpayment 
                                             ...
                                             and plots.project_id='".$key['id']."' and plotpayment.paid_date_temp LIKE '".date('Y-m-d')."'
                                        ) As installpayment";
                                     ...
                                     <td   style="text-align:right"><strong>
                                         <?php 
													$sql_d  ="SELECT
                                                                ...
                                                                installpayment.paid_date_temp LIKE'" .date('Y-m-d')."' && installpayment.fstatus !='Cancelled'";  
												...
												//	echo  round($tcash1,2);	
													...
													$sql_p  ="SELECT
                                                                ...
                                                                LEFT JOIN plots on plotpayment.plot_id=plots.id
                                                                where plots.project_id='".$key['id']."'and 
                                                                plotpayment.paid_date_temp LIKE'" .date('Y-m-d')."' && plotpayment.fstatus !='Cancelled'";  
		  ...
		  <?php
		   $sql_cash="SELECT SUM(total) as cash FROM (
                                            SELECT paidamount AS `total` FROM installpayment 
                                             LEFT JOIN plots on installpayment.plot_id=plots.id
                                             WHERE  (installpayment.payment_type LIKE'Cash') and (installpayment.fstatus !='Cancelled')
                                             and plots.project_id='".$key1['id']."' and installpayment.paid_date_temp LIKE'".date('Y-m-d')."'
                                            UNION ALL
                                            SELECT paidamount AS `total` FROM  plotpayment 
                                             LEFT JOIN plots on plotpayment.plot_id=plots.id
                                                WHERE  (plotpayment.paidas LIKE'Cash') and (plotpayment.fstatus !='Cancelled')
                                             and plots.project_id='".$key1['id']."' and plotpayment.paid_date_temp LIKE '".date('Y-m-d')."'
                                        ) As installpayment";        
