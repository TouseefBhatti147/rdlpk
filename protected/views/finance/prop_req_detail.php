...
?>
 <?php 
	$connection = Yii::app()->db; 	
$sql_details2  = "SELECT * FROM memberplot where id=".$_REQUEST['id']."";
$result_details2 = $connection->createCommand($sql_details2)->queryRow();
$sql_details3  = "SELECT * FROM memberplot where id=".$_GET['id'];
...
?>
   <div class="clearfix"></div>
    ...
    <div class="black-bg">Cost Of Plot:</div><div class="grey-bg"><?php if(!empty($result_details2['price'])){ echo $result_details2['price'];}  ?></div><br>
    ...
    <div class="black-bg">Comment:</div><div class="grey-bg"><?php  if(!empty($result_details3['comment'])){ echo $result_details3['comment'];} ?></div><br>
    <div class="black-bg"> Status:</div><div class="grey-bg"><?php  if(!empty($result_details3['status'])){ echo $result_details3['status'];} ?></div><br>
    </div> 
    <div class="span5 left-box">
  <h5 style="text-align:left;">Payment Details Link:</h5>
    <div class="black-bg">Link:</div><div class="grey-bg">
    <a href="prop_payment_details?id=<?php echo $result_details3['plot_id'] ?>&pid=<?php echo $key['project_id'] ?>">Payment Detail Link</a></div><br>
   
    </div> 
<?php 
if ((isset($result_details3['status'])|| (isset($result_details3['status']))!='approved')){ ?>
  <form action="Prop_submitstatus" method="post" enctype="multipart/form-data" onsubmit="return validateForm()" >

    <input type="hidden" value=<?php echo $result_details3['plot_id']?> name="plot_id" id="plot_id" class="f-left span4 clearfix" />
