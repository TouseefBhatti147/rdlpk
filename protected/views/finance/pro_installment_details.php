 ...
 echo '<b>Type&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;</b>' .$row['com_res'].'&nbsp;'; 
    ...
    echo $row['size'].'&nbsp;('.$row['plot_size'].')'.'</br>';
		...
		echo '<b>File No.</b>&nbsp;';
			...
			echo '&nbsp;<b>:</b>&nbsp;';if($row['stst']==2){ echo'<span style="color:red">Blocked</span>';}else{ echo $row['plot_detail_address'].'/'.$row['building_name'].'/'.$row['floor_name'].'</br>';}
...
<a href="#">Edit Installment Plan</a>
...
$gtotalsur=0;
	...
	{	
$i++;
  $due=floatval($due)+floatval($pay['dueamount']);
  $paid=$paid+floatval($pay['paidamount']);
   $duesurcharge=$duesurcharge+floatval($pay['surcharge']);
	...
	$oins=$due-$paid;
	 ...
	 $lastpaid=0;
	 $lastdued=0;
	 $timestamp = strtotime($pay['due_date_temp']); 
...
echo '<tr>';
  ...
  echo'<td rowspan="'.$co1.'">'.$pay['lab']. '</td>
     <td rowspan="'.$co1.'">'.$new_date.'</td>
     <td>';if($pay['paid_date_temp']=='0000-00-00'){ echo'';}else{ echo ($paid_date_temp);}echo '</td>
...
$gtotalsur=$gtotalsur+$totalduesur;
	 ...
	 if($pay['r_id']==0 and $pay['re_id']==0){echo'<a href="#">Edit</a>
	 /<a href="#" onclick="deletethis('.$_GET['id'].','.$pay['id'].')">Delete</a>';echo' / <a href="#">Pay</a> / <a href="#">Split</a></td>';}}
	 $id='';
	$id=$pay['id'];
	$lastdue=$pay['dueamount'];
	$lastpaid=$pay['paidamount'];
	$lastdued=$pay['due_date_temp'];
	 }?>
	<script>
...
echo '<td>'.$pay1['paid_date'].'</td>
	 ...
	 if(($lastdue) > 0 and $pay1['surcharge_re']==0 and $pay['paid_date_temp']!==$pay1['paid_date_temp']){
	  if($pay1['paid_date_temp']!==''){$curdate=$pay1['paid_date_temp'];}else{$curdate=date('Y-m-d');} 
	// $curdate=date('Y-m-d');
	...
	if(number_format($totalduesur)>0){echo '<b style="color:red;">'.number_format($totalduesur).'</b>';}else{ echo '<b style="color:red;">0</b>'; }
     ...
     <td>'.$pay1['remarks'].'</td>
     <td>';if(!empty($pay1['fstatus'])){if($pay1['fstatus']=='approved'){ echo '<b style="color:Green;">Verified</b>';}else{ echo '<b style="color:red;">'.$pay1['fstatus'].'</b>';}echo '/ <a href="#">Split</a>';} else{ 
	 if($pay1['r_id']==0 and $pay1['re_id']==0){echo'<a href="#">Edit</a>
	 /<a href="#" onclick="deletethis('.$_GET['id'].','.$pay1['id'].')">Delete</a>';echo' / <a href="#">Pay</a> / <a href="#">Split</a></td>';}}	}}
 $id=''; }
...
<?php if($row['stst']==2){ echo'<span style="color:red">Blocked</span>';}else{ 
	...
	echo '&nbsp; </br>'.$row['building_name'].'/'.$row['floor_name'];}
	//echo '<b>Date  :</b>' .$new_date.'</br>';
	?>
    &nbsp;</td>
...
echo '<tr>
  <td rowspan="'.$co1.'">'.$pay['lab']. '</td>
     <td rowspan="'.$co1.'">'.$pay['due_date_temp'].'</td>
     <td>'.$pay['paid_date'].'</td>
     <td rowspan="'.$co1.'" style="text-align:right">'.number_format($pay['dueamount']).'</td>
     <td style="text-align:right">'.number_format(floatval($pay['paidamount'])).'</td>
     <td>'.$pay['payment_type'].'</td>
...
foreach($payments as $pay1){
	...
	echo '<tr >';
if($pay1['paidamount']==''){$pay1['paidamount']=0;}     
echo '<td style="height:125px">'.$pay1['paid_date'].'</td>
     <td style="text-align:right">'.number_format($pay1['paidamount']).'</td>
     <td>'.$pay1['payment_type'].'</td>
     <td>';
	  if($pay1['r_id']>0){
