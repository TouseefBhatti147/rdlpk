...
<div class="shadow">
  ...
  <h3>Payment Details<div style="float:right;">
      <a href="pro_installment_details?id=<?php echo $_REQUEST['id']?> && pid=<?php echo $_REQUEST['id']?>">
...
if($row['type']=='file'){echo '<b>File Size &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  :&nbsp;</b>';}else {echo '<b>Plot Size &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;</b>';}
  echo $row['size'].'&nbsp;('.$row['plot_size'].')'.'</br>';
		...
		if($row['stst']==2){ echo'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:red">Blocked</span>';}else{echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>:</b>&nbsp;'.$row['plot_detail_address'].'/'.$row['building_name'].'/'.$row['floor_name'].'</br>';}
...
foreach($landcost as $land){
   ...
   if(is_numeric($tlcost)&&is_numeric($land['dueamount']))
   {
     $tlcost=$tlcost+$land['dueamount'];
   }
   		 if(is_numeric($tpcost)&&is_numeric($land['paidamount']))
   {
    $tpcost=$tpcost+$land['paidamount'];
   }
		
		
			if(is_numeric($land['surcharge'])) {
		$surch=$surch+$land['surcharge'];
			}else{
				$surch=$surch+0;
				}
				
		 if(is_numeric($paidsurch)&&is_numeric($land['paidsurcharge']))
		   {
...
$connection = Yii::app()->db;
		...
		if(!empty($discountr['discount'])){ $dis= $discountr['discount'];}
		if($land['paidamount']!=''){
			$tpayed++;
			}
...
echo '<tr><td>' . $i . '</td>
 ...
 <td style="text-align:right;">'.number_format($dis).'</td>
  ...
  <td>'.number_format($tpayed).'</td>
   <td><a href="#?id='.$_REQUEST['id'].'&& pid='.$_REQUEST['id'].'">Detail</a>/';
 if(Yii::app()->session['user_array']['per34']!='1'){echo'<a href="#">Discount</a>';}
        echo'</td>
</tr></tbody></table>  ';
...
<div style="float:right;"><a href="#"><h5>Add Charges/Fee</h5></a>
 <?php /// if(Yii::app()->session['user_array']['per35']='1'){?>
<a href="#">
 
 <h5>Add Surcharge</h5> </a><?php }?>
<a href="#"><h5>Charges List</h5> </a>
 </div><?php // }?>
...
$new_date = date('d-m-Y', $middle);
		...
		if(is_numeric($row['paidamount'])) {
		$paid=$paid+$row['paidamount'];
		}else{
			$duesurcharge=$duesurcharge+0;
			}
		...
		$duesurcharge=$duesurcharge+$row['surcharge'];
			...
			if(is_numeric($row['paidsurcharge'])) {
		$paidsurcharge=$paidsurcharge+$row['paidsurcharge'];
			}else{
					$paidsurcharge=$paidsurcharge+0;
				}
				if(is_numeric($row['surcharge'])&& is_numeric($row['paidamount'])) {
		$bsur=$row['amount']-$row['paidamount'];
		}else{
			$duesurcharge=$duesurcharge+0;
			}
		
		$ndis=$ndis+floatval($row['discount']);
	//	if($row['']
 ...
 <td style="text-align:right;">'.number_format($bsur-$row['discount']).'</td>
  <td>' .$due_date_temp. '</td>
  <td>';
  if($row['paid_date_temp']=='0000-00-00'){ echo'';}else{ echo ($paid_date_temp);} echo '</td>
...
echo'<a href="#">Edit</a>/ <a href="#">Pay</a>';
}
echo '</td></tr>	';} 
...
$new_date = date('d-m-Y', $middle); 
	...
	if($row['type']=='file'){
		...
		echo 'File No.&nbsp;';
			...
			if($row['stst']==2){echo'<span style="color:red">Blocked</span>';}else{echo '&nbsp;&nbsp;&nbsp;&nbsp;:&nbsp;'.$row['plot_detail_address'].'/'.$row['building_name'].'/'.$row['floor_name'].'</br>';}
...
foreach($landcost as $land){
	...
	if(is_numeric($tpcost)&&is_numeric($land['paidamount']))
   {
    $tpcost=$tpcost+$land['paidamount'];
		...
		$surch=$surch+floatval($land['surcharge']);
		if(is_numeric($paidsurch)&&is_numeric($land['paidsurcharge']))
   {
   	$paidsurch=$paidsurch+$land['paidsurcharge'];
   }
...
$new_date = date('d-m-Y', $middle);
		$i++;		
		$due=$due+$row['amount'];
		if(is_numeric($paid) && is_numeric($land['paidamount']))
			   {
				///$paid=$paid+$row['paidamount'];
			   }else {}
		
		$duesurcharge=$duesurcharge+floatval($row['surcharge']);
		$paidsurcharge=$paidsurcharge+floatval($row['paidsurcharge']);
		
		if(is_numeric($row['amount']) && is_numeric($row['paidamount']))
			   {
				   	$bsur=$row['amount']-$row['paidamount'];
				}
