...
$i++;
  ...
  $paid=floatval($paid)+floatval($pay['paidamount']);
    }
   $duesurcharge=floatval($duesurcharge)+floatval($pay['surcharge']);
    $paidsurcharge=floatval($paidsurcharge)+floatval($pay['paidsurcharge']);
	$oins=$due-$paid;
 	$co1=1;
  ...
  echo'<td rowspan="'.$co1.'">'.$pay['lab']. '</td>
	...
	echo ' </td>
     ...
     <?php 
	  ...
	  if($pay['paid_date_temp']!==''){$curdate=$pay['paid_date_temp'];}else{$curdate=date('Y-m-d');} 
	// $curdate=date('Y-m-d');
...
echo '<td>';if(($paid_date_temp1==null) || ($paid_date_temp1=='') || ($paid_date_temp1 == '30-11--0001' )) { echo''; } else{ echo $paid_date_temp1;}echo'</td>
...
foreach($payments as $pay1){
	...
	echo '<tr>';
if($pay1['paidamount']==''){$pay1['paidamount']=0;}     
echo '<td style="height:20px;">';if(($paid_date_temp1==null) || ($paid_date_temp1=='') || ($paid_date_temp1 == '30-11--0001' )) { echo''; } else{ echo $paid_date_temp1;}echo'</td>
     <td style="text-align:right">'.number_format($pay1['paidamount']).'</td>
     <td>'.$pay1['payment_type'].'</td>
     <td>';
	  if($pay1['r_id']>0){
...
$i++;
  ...
  $paid=floatval($paid)+floatval($pay['paidamount']);
   ...
   $duesurcharge=floatval($duesurcharge)+floatval($pay['surcharge']);
    $paidsurcharge=floatval($paidsurcharge)+floatval($pay['paidsurcharge']);
	$oins=floatval($due)-floatval($paid);
 	$co1=1;
...
foreach($payments as $pay1){
	...
	echo '<tr >';
if($pay1['paidamount']==''){$pay1['paidamount']=0;}     
echo '<td style="height:20px;">'.$pay1['paid_date'].'</td>
     <td style="text-align:right">'.number_format($pay1['paidamount']).'</td>
     <td>'.$pay1['payment_type'].'</td>
     <td>';
	  if($pay1['r_id']>0){
