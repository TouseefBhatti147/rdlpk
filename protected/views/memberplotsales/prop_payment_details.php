...
<div class="shadow">
  ...
  <h3>Payment Details<div style="float:right;"><a href="pro_installment_details?id=<?php echo $_REQUEST['id']?> && pid=<?php echo $_REQUEST['pid']?>"><h5>Installment Details</h5> </a></div>
</h3>
</div>
...
foreach($landcost as $land){
	$tlcost=floatval($tlcost)+floatval($land['dueamount']);
		$tpcost=floatval($tpcost)+floatval($land['paidamount']);
		$surch=floatval($surch)+floatval($land['surcharge']);
		$paidsurch=floatval($paidsurch)+floatval($land['paidsurcharge']);
...
$new_date = date('d-m-Y', $middle);
	 ...
	 $paid_date_temp = date('d-m-Y', $paid_date_temp);
		...
		$due=floatval($due)+floatval($row['amount']);
		$paid=floatval($paid)+floatval($row['paidamount']);
		$duesurcharge=floatval($duesurcharge)+floatval($row['surcharge']);
		$paidsurcharge=floatval($paidsurcharge)+floatval($row['paidsurcharge']);
		$bsur=floatval($row['amount'])-floatval($row['paidamount']);
		$ndis=floatval($ndis)+floatval($row['discount']);
