...
<!--<div class="alert alert-info" style="float:right;">
     ...
     <label style="float:left;"><input type="radio" name="pptype" value="R" class="new1" style="float:left;"  onclick="document.getElementById('project_id').disabled = false;" />Residential  </label>  
     <label style="float:left;"><input type="radio" name="pptype" value="C"  class="new1" style="float:left;" onclick="document.getElementById('project_id').disabled = false;"/>Commercial</label>
    </div>--->
    <div class="clearfix"></div>
     <div class="float-left" >
      ...
      //     {
		 ...
		 //  });
		   ...
		   var sid=$("#pptype").val();
			...
			$.ajax({
      type: "POST",
      url:    "ajaxRequest1233?val1="+id,
	  contenetType:"json",
      success: function(jsonList){var json = $.parseJSON(jsonList);
		var listItems='';
	$(json).each(function(i,val){
	listItems+= "<option value='" + val.id + "'>" + val.size +"  ("+ val.dimension +") </option>";
    });listItems+="";
	$("#size2").html(listItems);
          }
     	});}
...
$.ajax({
	 ...
	 ///// url:    "AjaxRequest1?pro="+pro+"&size="+size+"&floor_id="+floor_id+"&&building_id="+building_id,
	  url:    "AjaxRequest100?pro="+pro+"&size="+size+"&floor_id="+floor_id+"&building_id="+building_id,
	  contenetType:"json",
      success: function(jsonList){var json = $.parseJSON(jsonList);
		var listItems='';
...
$.ajax({
      type: "POST",
	    // url:    "AjaxRequest9?val1="+pro+"&&building_id="+building_id,
      url:    "AjaxRequest70?val1="+id+"&size="+size+"&pro="+pro+"&building_id="+building_id,
	  contenetType:"json",
      success: function(jsonList){var json = $.parseJSON(jsonList);
var listItems='';
