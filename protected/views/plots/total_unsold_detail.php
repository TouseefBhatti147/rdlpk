...
<head>
    <script type="text/javascript">
        function PrintDiv(id) {
            ...
            var myWindow = window.open('', 'Unsold Plots', 'height=400,width=600');
            myWindow.document.write('<html><head><title>Unsold Summary</title>');
            ...
            <table class="table table-striped table-new table-bordered">
				...
				if(empty($_GET['block_id'])){
                   $opensql  = "SELECT
								...
								Inner JOIN size_cat  ON (size_cat.id = plots.size2)
where size2='".$_GET['size']."' AND com_res='".$com_res."' and plots.project_id='".$_GET['project_id']."'and plots.status='' and type !='file' order by sectors.sectors_sorting,streets.streets_sorting,plots.plot_detail_address asc";
				}else{
				    $opensql  = "SELECT
								plots.id
								, plots.street_id
								...
								Inner JOIN sectors  ON (plots.sector = sectors.id)
								Inner JOIN size_cat  ON (size_cat.id = plots.size2)
where size2='".$_GET['size']."' AND com_res='".$com_res."' and plots.project_id='".$_GET['project_id']."'and (plots.block_id='".$_GET['block_id']."') and plots.status='' and type !='file' order by sectors.sectors_sorting,streets.streets_sorting,plots.plot_detail_address asc";
				}
	 			 $openres = $connection->createCommand($opensql)->queryAll();
				 foreach($openres as $unsold){
					 $i++;
					 $sqlcat="SELECT categories.id,(SELECT NAME FROM categories Inner JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 6 and cat_plot.plot_id='".$unsold['id']."'
) AS corner,(SELECT NAME FROM categories Inner JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 7 and cat_plot.plot_id='".$unsold['id']."') AS fp,
...
Inner JOIN cat_plot ON cat_plot.cat_id=categories.id
 where cat_plot.plot_id='".$unsold['id']."'";
	$rescat = $connection->createCommand($sqlcat)->queryRow();
                ...
                <td><?php if(!empty($rescat['corner'] || isset($rescat['corner']) ) && ($rescat['corner']=='Corner')){ echo'<img src="/images/tickmark.png" />';}else {echo'';}?></td>
				<td><?php if(!empty($rescat['fp'] || isset($rescat['fp'])) && ($rescat['fp']=='Facing Park')){ echo'<img src="/images/tickmark.png" />';}else {echo'';}?></td>
                <td><?php if(!empty($rescat['blvd'] || isset($rescat['blvd'])) && ($rescat['blvd']=='Boulevard')){ echo'<img src="/images/tickmark.png" />';}else {echo'';}?></td>
				<td><?php if(!empty($rescat['80feet'] || isset($rescat['80feet'])) && ($rescat['80feet']=='Boulevard (80 Feet)')){ echo'<img src="/images/tickmark.png" />';}else {echo'';}?></td>
				<td><?php if(!empty($rescat['60feet'] || isset($rescat['60feet'])) && ($rescat['60feet']=='Road (60 Feet)')){ echo'<img src="/images/tickmark.png" />';}else {echo'';}?></td>
          ...
          <tr>
            <td bgcolor="#FFFFFF">
                ...
                <tr>
                  ...
                  <td colspan="10" align="center" valign="middle" style="border-top:thin; border-top-style:solid; border-right:thin; border-right-style:solid; font-size:14px;  font-weight:bold"><span class="style6">Total Unsold Plots</span></td>
...
where size2='".$_GET['size']."' AND com_res='".$com_res."' and plots.project_id='".$_GET['project_id']."' and plots.status='' and type !='file' order by sectors.sectors_sorting,streets.streets_sorting,plots.plot_detail_address asc";
	 			 $openres = $connection->createCommand($opensql)->queryAll();
			         $i=0;
                	 foreach($openres as $unsold){
                	     	$sqlcat1="SELECT categories.id,
(SELECT NAME FROM categories Inner JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 6 and cat_plot.plot_id='".$unsold['id']."') AS corner,
