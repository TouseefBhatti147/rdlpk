...
where size2='".$_GET['size']."' AND com_res='".$com_res."' and plots.project_id='".$_GET['project_id']."' and plots.ctag='HRL Reserved' and plots.status='' and plots.type !='file' order by sectors.sectors_sorting,streets.streets_sorting,plots.plot_detail_address asc";
...
LEFT JOIN cat_plot ON cat_plot.cat_id=categories.id
 where cat_plot.plot_id='".$unsold['id']."'";
	$rescat = $connection->createCommand($sqlcat)->queryRow();
                ...
                <td><?php if(!empty($rescat['blvd']) && ($rescat['blvd']=='Boulevard')){ echo'<img src="/images/tickmark.png" />';}else {echo'';}?></td>
