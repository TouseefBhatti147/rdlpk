...
foreach($result_plots as $row1)
{
  ...
  $opensql  = "SELECT * from plots
	 where size2='".$row1['id']."' AND com_res='".$_POST['com_res']."' and project_id='".$_POST['project']."' and type='Plot'";
...
echo'</td>
<td style="text-align:right"><a href="total_plots_detail?com_res='.$com_res.'&size='.$row1['id'].'&&project_id='.$_POST['project'].'">'.count($openres).'</a></td>';
 $resvsql  = "SELECT * from plots
	 where size2='".$row1['id']."' AND com_res='".$_POST['com_res']."' and project_id='".$_POST['project']."'   and status='Alotted' and type='file'";
	 	$resvres = $connection->createCommand($resvsql)->query();
echo'<td style="text-align:right"><a href="hrl_reserved_plots_detail?com_res='.$com_res.'&size='.$row1['id'].'&&project_id='.$_POST['project'].'">'.count($resvres).'</a></td>';
$alsql  = "SELECT * from plots
	 where size2='".$row1['id']."' AND com_res='".$_POST['com_res']."' and project_id='".$_POST['project']."' and status='Alotted' and type='Plot'";
	 	$alres = $connection->createCommand($alsql)->query();
echo '<td style="text-align:right"><a href="against_land_plots_detail?com_res='.$com_res.'&size='.$row1['id'].'&&project_id='.$_POST['project'].'">'.count($alres).'</a></td>';
$villassql  = "SELECT * from plots
	 where size2='".$row1['id']."' AND com_res='".$_POST['com_res']."' and project_id='".$_POST['project']."'  and  `ctag` LIKE '%Villas%' and status='' and type='Plot'";
	 	$villasres = $connection->createCommand($villassql)->query();
echo '<td style="text-align:right"><a href="reserved_villas_detail?com_res='.$com_res.'&size='.$row1['id'].'&&project_id='.$_POST['project'].'">'.count($villasres).'</a></td>';
$total=count($openres)-(count($resvres)+count($alres)+count($villasres));
echo'<td style="text-align:right"><a href="net_balance_plots_detail?com_res='.$com_res.'&size='.$row1['id'].'&&project_id='.$_POST['project'].'">'.$total.'</a></td><td></td><td></td></tr>';
}
 ?>
</tbody>
</table>
...
<input style="float:left;" type="submit" name="submit" value="Generate PDF" />
</form>
</form>
