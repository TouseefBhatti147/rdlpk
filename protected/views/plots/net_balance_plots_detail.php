...
<h3>Net Balance Plots Detail</h3>
...
where size2='".$_GET['size']."' AND com_res='".$com_res."' and plots.project_id='".$_GET['project_id']."' and plots.status='' and type  !='file' and ctag NOT IN('Against Land','HRL Reserved','Villas') order by sectors.sectors_sorting,streets.streets_sorting,plots.plot_detail_address asc";
	 			 $res_al = $connection->createCommand($against_land)->query();
				 foreach($res_al as $al){
					 $i++;
					$sqlcat="SELECT categories.id,(SELECT NAME FROM categories LEFT JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 6 and cat_plot.plot_id='".$al['id']."'
) AS corner,(SELECT NAME FROM categories LEFT JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 7 and cat_plot.plot_id='".$al['id']."') AS fp,
(SELECT NAME FROM categories LEFT JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 8 and cat_plot.plot_id='".$al['id']."') AS blvd,
(SELECT NAME FROM categories LEFT JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 9 and cat_plot.plot_id='".$al['id']."') AS 80feet,
(SELECT NAME FROM categories LEFT JOIN cat_plot ON cat_plot.cat_id = categories.id WHERE cat_id = 10 and cat_plot.plot_id='".$al['id']."') AS 60feet
from categories
LEFT JOIN cat_plot ON cat_plot.cat_id=categories.id
 where cat_plot.plot_id='".$al['id']."'";
	$rescat = $connection->createCommand($sqlcat)->queryRow();
				?>
                <tr>
                  <td><?php echo $i;?></td>
            
                <td><?php echo $al['project_name'];?></td>
                <td><?php echo $al['size'];?></td>
                <td><?php echo $al['plot_detail_address'];?></td>
                <td><?php echo $al['street'];?></td>
                <td><?php echo $al['sector_name'];?></td>
          ...
          <tr>
            <td bgcolor="#FFFFFF">
                ...
                <tr>
                  ...
                  <td colspan="10" align="center" valign="middle" style="border-top:thin; border-top-style:solid; border-right:thin; border-right-style:solid; font-size:14px;  font-weight:bold"><span class="style6">Net Balance Plots</span></td>
