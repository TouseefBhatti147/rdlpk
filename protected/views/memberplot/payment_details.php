...
$tbalance=0;
		...
		foreach($landcost as $land){
			$tlcost=$tlcost+floatval($land['dueamount']);
			if($land['fstatus']!='Bounce'){
				$tpcost=floatval($tpcost)+floatval($land['paidamount']);
				
			
			   }
			
				$surch=$surch+floatval($land['surcharge']);
				$paidsurch=$paidsurch+floatval($land['paidsurcharge']);
		...
		$connection = Yii::app()->db;
				...
				$discountr = $connection->createCommand($discount)->queryRow();
				 if(empty($discountr['discount'])){ $dis="";}else { $dis=$discountr['discount'];}
				if($land['paidamount']!=''){
					$tpayed++;
					}
...
$new_date = date('d-m-Y', $middle);
		...
		$due=$due+floatval($row['amount']);
		$paid=$paid+floatval($row['paidamount']);
		$duesurcharge=$duesurcharge+floatval($row['surcharge']);
		$paidsurcharge=$paidsurcharge+floatval($row['paidsurcharge']);
		$bsur=floatval($row['amount'])-floatval($row['paidamount']);
 ...
 <td style="text-align:right;">' .$row['paidamount']. '</td>
  ...
  <td style="text-align:right;">'.floatval($row['discount']).' </td>
 <td style="text-align:right;">'.floatval($bsur-$row['discount']).'</td>
  <td>';
  if($row['due_date_temp']=='0000-00-00' || $row['due_date_temp']== null){ echo'';}else{ echo ($due_date_temp);} echo '</td>
  <td>';
  if($row['paid_date_temp']=='0000-00-00' || $row['paid_date_temp']== null){ echo'';}else{ echo ($paid_date_temp);} echo '</td>
  <td style="text-align:right;">' ;if($row['surcharge']==0) { echo $row['surcharge'];}echo '</td>
    <td style="text-align:right;">' . $row['paidsurcharge'] . '</td>
   <td>';
if($row['r_id']>0){
    $connection = Yii::app()->db;
	  $re1 = "SELECT slipno,r_no,jvno FROM rpt_print where rid='".$row['r_id']."' and msid='".$row['plot_id']."'";
		$rec1=$connection->createCommand($re1)->queryRow();	
		echo $rec1['slipno'].'/<b style="color:blue;">'.$rec1['r_no'];
		if($rec1['jvno'] > 1 ){echo 'JV-'.$rec1['jvno'];}
		echo '</b>';
		}elseif($row['re_id']>0){
  		$re = "SELECT slipno,r_no,jvno FROM rpt_print where id='".$row['re_id']."'";
...
if($row['r_id']==0 and $row['re_id']==0 and ($row['fstatus']!='approved') and (Yii::app()->session['user_array']['id']=='1')){
    echo'<a href="delete_charges?pid='.$_REQUEST['id'].'&&id='.$row['id'].'&&mem_id='.$mem['member_id'].'">Delete </a>';
}else{ echo'';}
echo '</td></tr>	';} 
...
<tr>
 ...
 <?php $res=array();
    ...
    foreach($members as $mem){
		...
		foreach($landcost as $land){
			        $tlcost=$tlcost+floatval($land['dueamount']);
				$tpcost=$tpcost+floatval($land['paidamount']);
				$surch=$surch+floatval($land['surcharge']);
				$paidsurch=$paidsurch+floatval($land['paidsurcharge']);
				$bamount=$tlcost-$tpcost;
		$tbalance=0;
