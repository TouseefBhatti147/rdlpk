...
if(empty($discountr['discount'])){$discountr['discount']=0;}
?> 
...
$paidsurcharge=0;
?>
</div> <?php ///if(Yii::app()->session['user_array']['id']=='1')
	{?>
<a href="http://rdlpk.com/index.php/memberplot/installment_edit?id=<?php echo $_REQUEST['id']; ?>">
...
$gtotalsur=0;
 ...
 $i++;
   $due=$due+floatval($pay['dueamount']);
	 if($pay['fstatus']!='Bounce'){
   $paid=$paid+floatval($pay['paidamount']);
	 }
...
echo '<td>';
  if(($paid_date_temp1==null) || ($paid_date_temp1=='0000-00-00') || ($paid_date_temp1 == '30-11--0001' )) { echo''; } else{ echo $paid_date_temp1;}echo'</td>
	 ...
	 if(($lastdue) > 0 and $pay1['surcharge_re']==0 and $pay1['paid_date_temp']!==$pay1['paid_date_temp']){
	  if($pay1['paid_date_temp']!=='0000-00-00'){$curdate=$pay1['paid_date_temp'];}else{$paiddate=date("Y-m-d");} 
	// $curdate=date('Y-m-d');
	...
	/// if($surchargedur>1){
	///echo $surchargedur.'dur';	 
	 $totalduesur=floatval($surchargedur)*floatval($surchargeratio);
	 ///}else{$totalduesur=0;}	
	if(number_format($totalduesur)>0){echo '<b style="color:red;">'.number_format($totalduesur).'</b>';}else{ echo '<b style="color:red;">0</b>'; }
	 $lastpaid=$lastpaid+$pay1['paidamount'];
	 $gtotalsur=$gtotalsur+$totalduesur;
...
if($charg['r_id']==0 and $charg['re_id']==0){
echo'<a href="up_charges?pid='.$_REQUEST['id'].'&&id='.$charg['id'].'&&mem_id='.$mem['member_id'].'">Edit/</a>/ <a href="update_charges?pid='.$_REQUEST['id'].'&&id='.$charg['id'].'&&mem_id='.$mem['member_id'].'">Pay/</a>';
}
if($charg['r_id']==0 and $charg['re_id']==0 and ($charg['fstatus']!='approved') and (Yii::app()->session['user_array']['id']=='1')){
    echo'<a href="delete_charges?pid='.$_REQUEST['id'].'&&id='.$charg['id'].'&&mem_id='.$mem['member_id'].'">Delete </a>';
}else{ echo'';}
echo '</td></tr>	';} 
...
<td style="text-align:right; font-weight:bold;"><?php echo number_format((floatval($price)+floatval($row['extra_land_price'])+floatval($row['PLcharges']))-floatval($discountr['discount']))?></td><td > </td>
</tr>
  ...
  <td rowspan="'.$co1.'">'.$pay['lab']. '</td>
	...
	echo ' </td>
	 ...
	 if($pay['dueamount'] > 1 and $pay['surcharge_re']==00){
	  if($pay['paid_date_temp']!=='0000-00-00'){$curdate=$pay['paid_date_temp'];}else{$curdate=date('Y-m-d');} 
	// $curdate=date('Y-m-d');
      if(empty($pay['paidamount']) || $pay['paidamount']=='')
			{
			$surchargeratio=$pay['dueamount']/100*0.05;
...
echo '<td style="height:20px;">';if(($paid_date_temp1==null) || ($paid_date_temp1=='0000-00-00') || ($paid_date_temp1 == '30-11--0001' )) { echo''; } else{ echo $paid_date_temp1;}echo'</td>
	...
	echo '<td align="right">';
	 ...
	 if(($lastdue) > 0 and $pay1['surcharge_re']==0 and $pay['paid_date_temp']!==$pay1['paid_date_temp']){
	  if($pay1['paid_date_temp']!=='0000-00-00'){$curdate=$pay1['paid_date_temp'];}else{$curdate=date('Y-m-d');} 
	// $curdate=date('Y-m-d');
...
echo '<tr>
  <td rowspan="'.$co1.'">'.$pay['lab']. '</td>
	...
	echo ' </td>
     ...
     <?php 
	 if($pay['dueamount'] > 1 and $pay['surcharge_re']==0){
	  if($pay['paid_date_temp']!=='0000-00-00'){$curdate=$pay['paid_date_temp'];}else{$curdate=date('d-m-Y');} 
	// $curdate=date('Y-m-d');
     $surchargeratio=$pay['dueamount']/100*0.05;
	 $duedate=$pay['due_date'];
	 if($pay['paid_date_temp']!=='0000-00-00'){$paiddate=$pay['paid_date_temp'];}else{$paiddate=date('d-m-Y');} 
	 $datetime1 = new DateTime($duedate);
	 $datetime2 = new DateTime($paiddate);
	 $interval = $datetime1->diff($datetime2); 
	 $surchargedur= $interval->format('%R%a ');
...
echo '<td style="height:20px;">'.$pay1['paid_date_temp'].'</td>
	 ...
	 echo '<td align="right">';
	  ...
	  if($pay1['paid_date_temp']!=='0000-00-00'){$curdate=$pay1['paid_date_temp'];}else{$curdate=date('d-m-Y');} 
	// $curdate=date('Y-m-d');
...
echo '</td>
<td>';if(!empty($charg['fstatus'])){if($charg['fstatus']=='approved'){ echo '<b style="color:Green;">Verified</b>';}else{echo '<b style="color:Green;">'.$charg['fstatus'].'</b>';}
