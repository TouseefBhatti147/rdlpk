...
where  installpayment.r_id='".$_REQUEST['id']."' and  
		(installpayment.fstatus not in ( 'Cancelled','Rejected') and installpayment.fstatus is null or installpayment.fstatus='')";
...
if($ch['ref']==0){
if($ch['dueamount']==''){$dueam=0;}
}else
{
  $sql_amounts  = "Select sum(dueamount) as da,sum(paidamount) as pa from installpayment where ref='".$ch['ref']."'|| id='".$ch['ref']."'  ";
  $result_amounts = $connection->createCommand($sql_amounts)->queryRow();
  $dueam=$result_amounts['da']-$result_amounts['pa'];
}
	if($ch['paidamount']==''){$ch['paidamount']=0;}
	if($ch['surcharge']==''){$ch['surcharge']=0;}
	if($ch['paidsurcharge']==''){$ch['paidsurcharge']=0;}
...
$result_ref = $connection->createCommand($sql_ref)->queryRow();	
	...
	$ch['due_date_temp']=$result_ref['due_date_temp'];
	}
echo '<tr>
<td>'.$ch['plotno'].'/'.$ch['app_no'].'</td>
<td>'.$ch['lab'].'</td>
<td>'.$ch['due_date_temp'].'</td>
<td style="text-align:right;">'.number_format($ch['paidamount']).'</td>
<td style="text-align:right;">'.number_format($ch['paidamount']).'</td>
...
?>
  <select style="width: 150px;" name="plots1" id="plots1">
    <?php 
$connection = Yii::app()->db; 
echo $sql_plot  = "SELECT *,plots.id as pid from plots
Left join memberplot on (plots.id=memberplot.plot_id)
Left join projects on (projects.id=plots.project_id)
Left join streets on (streets.id=plots.street_id)
Left join sectors on (sectors.id=plots.sector)
...
$result_ref = $connection->createCommand($sql_ref)->queryRow();	
	...
	$ch1['due_date_temp']=$result_ref['due_date_temp'];
	}
if($ch1['plot_id']==$new['plot_id']){
if($ch1['paidamount']==''){$ch1['paidamount']=0;}
$no=$no+1;
...
<td style="width:210px !important;"> '.$ch1['remarks'].'</td>
<td style="width:95px !important; text-align:right;">'.number_format($ch1['paidamount']).'</td>
<td style="width:95px !important; text-align:right;">'.number_format($ch1['paidamount']).'</td></tr>';}
}
echo '</tbody></table>';
$totalw=word($total);
...
echo '<div style="margin: '.(408+$heg).'px 50px 0px -22px;position: absolute; font-size:10px;"><b>Username:</b>'.$result_us['salesname'].'&nbsp;|&nbsp;'.Yii::app()->session['user_array']['firstname'].'&nbsp;'.Yii::app()->session['user_array']['middelname'].'&nbsp;'.Yii::app()->session['user_array']['lastname'].'&nbsp;|&nbsp;'.date('d-M-Y h:m:s').'&nbsp;&nbsp;Form #'.$result_membership['app_no'].'</div>';
echo '</div>';
echo '</tbody></table>';
//2st copy
