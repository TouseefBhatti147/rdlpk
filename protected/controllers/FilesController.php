...
where plots.id='" . $_REQUEST['id'] . "'";
    ...
    public function actionCreate() {
        ...
        if (Yii::app()->session['user_array']['per4'] == '1') {
            ...
            if (isset($_POST['file_size']) && empty($_POST['file_size'])) {
             ...
             $sq = "SELECT * from plots where project_id='" . $_POST['project_id'] . "'and size2='" . $_POST['size2'] . "'and block_id='" . $_POST['block'] . "' AND sector='" . $_POST['sector'] . "' AND street_id='" . $_POST['street_id'] . "' AND plot_detail_address='" . $_POST['file_detail_address'] . "'";
 ...
 (type,project_id,block_id,street_id, plot_detail_address, plot_size, size2, com_res,price,sector,create_date )

               	    	  VALUES ( "file","' . $_POST['project_id'] . '","' . $_POST['block'] . '", "' . $_POST['street_id'] . '", "' . $_POST['file_detail_address'] . '", "' . $_POST['file_size'] . '", "' . $_POST['size2'] . '", "' . $_POST['com_res'] . '", "' . $_POST['price'] . '", "' . $_POST['sector'] . '","' . date('Y-m-d h:i:s') . '" )';
                $command = $connection->createCommand($sql);
                $command->execute();
...
////////////////////////history/////////
   ...
   public function actionSearchreq()
    {
        if ((Yii::app()->session['user_array']['per2'] == '1') || (Yii::app()->session['user_array']['per34'] == '1')|| (Yii::app()->session['user_array']['id'] == '85')) {
            $com_res = '';
            $size = '';
            $where = '';
            $and = false;
            ...
            if (isset($_POST['block']) && $_POST['block'] != "") {
                 if ($and == true) {
                $where .= "and plots.block_id LIKE '%" . $_POST['block'] . "%'";
                 }else{ $where .= "plots.block_id LIKE '%" . $_POST['block'] . "%'";}
                $and = true;
                $block = $_POST['block'];
            }
            if ($and == true) {
            ...
            if (!empty($_POST['stat'])) {
                ...
                if ($_POST['stat'] == 2) {
                    $where .= "and plots.status ='Alotted'";
                }

                if ($_POST['stat'] == 3) {
                    $where .= "and plots.status =''";
                }

                if ($_POST['stat'] == 4) {
                    $where .= "and plots.bstatus ='reserved'";
                }
                if ($_POST['stat'] == 5) {
                    $where .= "and plots.status ='Requested(T)'";
                }
                if ($_POST['stat'] == 6) {
	...
	Inner JOIN blocks  ON (plots.block_id = blocks.id)
...
where $where and plots.type='file' limit $start,$limit";
            ...
            if ($result_members != '') {
                ...
                foreach ($result_members as $key) {
                    ...
                    echo '<tr><td>' . $key['plotno'] . '</td><td>' . $key['project_name'] . '</td><td>' . $key['block_name'] . '</td><td>' . $key['street'] . '</td><td><a href="' . $home . '/index.php/user/filehistory?id=' . $key['id'] . '">' . $key['plot_detail_address'] . '</a></td><td>' . $key['plot_size'] . '</td><td>' . $key['size'] . '</td><td>' . $key['com_res'] . '</td><td>' . $key['sector_name'] . '</td><td>';
                    if (Yii::app()->session['user_array']['per34'] == '1' || Yii::app()->session['user_array']['id'] == '85') {
                        echo 'Convert';
                    } else {
                        echo '<a href="convert2plot?id=' . $key['id'] . '">Convert</a>';
                    }
                    echo '</td>';
                    if ((Yii::app()->session['user_array']['per1'] == '1' || Yii::app()->session['user_array']['id'] == '85') ) {
                        echo '<td>';
                        $F = '';
                        if ($key['status'] == '') {
                            if (Yii::app()->session['user_array']['per34'] == '1' || Yii::app()->session['user_array']['id'] == '85') {
                            ...
                            echo '<a href="' . $home . '/index.php/memberfile/requested_detail?id=' . $key['id'] . '">' . "Requested" . '(' . $M . ')' . '</a>';
                        } elseif ($key['status'] == 'Requested(T)') {
                            echo '<a  href="' . $home . '/index.php/memberfile/treq_detail?id=' . $key['id'] . '">' . $key['status'] . '</a>';
                        } else {
                            echo $key['status'];
                        }
...
<?php // for pagination 
            ...
            echo '<tr><td colspan="13">' . $pagination . '</td></tr>';
            exit;
            // for pagination END 
            exit;
...
where plots.id='" . $_REQUEST['id'] . "' AND type='file'";
    ...
    public function actionAjaxRequest($pro, $blk) {
        $connection = Yii::app()->db;
        ////$sql_street = "SELECT * from sectors where project_id='" . $val1 . "'";
		 $sql_street = "SELECT sectors.* from sectors where project_id='" . $pro . "' and block_id='" . $blk . "'";
        $result_streets = $connection->createCommand($sql_street)->query();
        $street = array();
        foreach ($result_streets as $str) {
            $street[] = $str;
